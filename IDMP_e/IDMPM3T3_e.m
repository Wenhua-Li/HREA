function varargout = IDMPM3T3_e(Operation,Global,input)
% <problem> <IDMP_e>
    ...
    switch Operation
        ...
        case 'value'
            ...
            for i=1:Global.M
                ...
                temp(:,1) = temp(:,1)+100.*(PopDec(:,3)+.6).^2;               
                t2 = PopDec(:,1)-.5 + PopDec(:,2)+.5;                
                temp(:,2) = temp(:,2)+100.*(PopDec(:,3)+.2 + .1.*t2 ).^2;              
                t3 = PopDec(:,1)-.5 + PopDec(:,2)-.5;                
                temp(:,3) = temp(:,3)+100.*(PopDec(:,3)-.2 + .2.*t3 ).^2;              
                t4 = PopDec(:,1)+.5 + PopDec(:,2)-.5;                
                temp(:,4) = temp(:,4)+100.*(PopDec(:,3)-.6 + .3.*t4 ).^2;
                PopObj(:,i) = min(temp,[],2);
            end            
