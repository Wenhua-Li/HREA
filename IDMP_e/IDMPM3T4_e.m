function varargout = IDMPM3T4_e(Operation,Global,input)
% <problem> <IDMP_e>
    ...
    switch Operation
        ...
        case 'value'
            ...
            for i=1:Global.M
                ...
                temp(:,1) = temp(:,1)+100.*(1-cos(1.*2.*pi.*(PopDec(:,3)+.6)));
                temp(:,2) = temp(:,2)+100.*(1-cos(1.*2.*pi.*(PopDec(:,3)+.2)));
                temp(:,3) = temp(:,3)+100.*(1-cos(2.*pi.*(PopDec(:,3)-.2)));
                temp(:,4) = temp(:,4)+100.*(1-cos(2.*pi.*(PopDec(:,3)-.6)));
                
%                 temp(:,1) = temp(:,1)+100.*((PopDec(:,3)+.6).^2+1-cos(1.*2.*pi.*(PopDec(:,3)+.6)));
%                 temp(:,2) = temp(:,2)+100.*((PopDec(:,3)+.2).^2+1-cos(2.*2.*pi.*(PopDec(:,3)+.2)));
%                 temp(:,3) = temp(:,3)+100.*((PopDec(:,3)-.2).^2+1-cos(3.*2.*pi.*(PopDec(:,3)-.2)));
%                 temp(:,4) = temp(:,4)+100.*((PopDec(:,3)-.6).^2+1-cos(4.*2.*pi.*(PopDec(:,3)-.6)));
                PopObj(:,i) = min(temp,[],2);
            end            
